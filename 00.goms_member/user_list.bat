...
(
    ...
    for /l %%i in (0, 1, 10) do (
        ...
        if defined alias (
	    if "!alias!"=="regdate" (
		echo     @DateTimeFormat(pattern = "yyyy-MM-dd hh:mm:ss"^)	
		echo     @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd hh:mm:ss", timezone = "Asia/Seoul"^)
	    )
            echo     private !type! !alias!;
        )
    )
...
(
    ...
    echo.
    echo    @Tag^(name = "GOMS API > %uCategory%", description = "GOMS API %uCategory% All Query."^)
    ...
    echo    }
    echo.
    echo    @Tag^(name = "GOMS API > %uCategory%", description = "GOMS API %uCategory% query"^)
    ...
    echo    }
    echo.
    echo    @Tag^(name = "GOMS API > %uCategory%", description = "GOMS API %uCategory% delete"^)
    ...
    echo    }
    echo.
    echo    @Tag^(name = "GOMS API > %uCategory%", description = "GOMS API %uCategory% update"^)
    ...
    echo    }
    echo.
    echo    @Tag^(name = "GOMS API > %uCategory%", description = "GOMS API %uCategory% insert."^)
...
set "dtd=<^!DOCTYPE mapper PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN" "http://mybatis.org/dtd/mybatis-3-mapper.dtd">"
...
:: ############################  Mapper Del XML ############################
:: Create the file and write content to it >>>> MemberDelInDtoMapper.xml >>> delete > not resultType
(
    echo ^<?xml version="1.0" encoding="UTF-8"?^>
    echo !dtd!
    echo ^<mapper namespace="%strRepositoryPackageName%.%strRepositoryClassName%"^>
